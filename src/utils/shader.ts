...
export class ShaderUtil implements IShaderUtil {
    ...
    public async CreateShaderProgram() : Promise<WebGLProgram | null> {
        const vertShader = await this.createShader(this.gl.VERTEX_SHADER, this.vertexSource);
        if(vertShader === null) return null;

        const fragShader = await this.createShader(this.gl.FRAGMENT_SHADER, this.fragmentSource);
        ...
        const isSuccess = this.gl.getProgramParameter(shaderProgram, this.gl.LINK_STATUS);
    ...
    private async createShader(type: number, source: string) : Promise<WebGLShader | null> {
        ...
        const isSuccess = this.gl.getShaderParameter(shader, this.gl.COMPILE_STATUS);
